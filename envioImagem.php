<?php
	...
	if (isset($_FILES['imagem'])) {
		...
		if ($img != NULL) {//se a imagem for carregada corretamente
			...
			if(move_uploaded_file($img['tmp_name'], $nomeFinal)){//move a imagem para a pasta do sistema com o nome predefinido anteriormente (nomeFinal), se der certo ele continua o código abaixo.

				$tamanhoImg = filesize($nomeFinal);//tamanho da imagem cadastrada

				$mysqlImg = addslashes(fread(fopen($nomeFinal, 'r'), $tamanhoImg));

				$query = "insert into cenario (codcliente, imgCenario, tipo_img) values (106045, '$mysqlImg', '$tipo')";
